...
| **网络诊断** | **DNS(rDNS)解析**、**DNS 记录查询(A/MX/CNAME/TXT/NS)**、**Whois(域名/IP 注册信息)**、**CIDR 计算器**、HTTP(S) 检查、TLS 证书、Ping 质量、路径 MTU、**IP 归属地**；全面支持 **IPv6** |
...
| **加解密编码** | 密码生成（`secrets` 安全随机）+ Base64/Base64URL/URL/Hex/HTML/Unicode 编解码、MD5/SHA1/SHA256/SHA512、JSON 格式化、时间戳换算、进制转换、摩尔斯、ROT13 |
| **生活** | 单位换算（长度/面积/体积/质量/温度/速度/时间/功率/压力/能量/密度）、几何体计算、个税、贷款、存款、利率换算、日期计算、数字转大写金额、星座/生肖/血型遗传、纸张尺寸、世界时间等 |
| **🔍 功能搜索** | 顶栏搜索框，输入关键词即可快速定位并跳转到对应功能标签页 |
